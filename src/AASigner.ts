...
// Only works on goerli. Address would be same on other networks
export const fallbackHandlerAddress = '0xda0cdf6878e338152C614Cd075d4e6ff857Aa060'
...
export class AASigner extends Signer {
  ...
  async connectWalletAddress (address: string): Promise<void> {
    ...
    this._wallet = SmartWallet__factory.connect(address, this.signer)
  ...
  async _deploymentAddress (): Promise<string> {
    ...
    // Note: could be obtained from Entry point static call as well
    // @notice index used here is 0
    const ownerAddress = await this.signer.getAddress()
    const deploymentAddress = await new WalletFactory(this.provider!).getCounterFactualAddress(ownerAddress, 0)
    console.log('counterfactual deployment address ', deploymentAddress)
  ...
  async getWallet (): Promise<SmartWallet> {
  ...
  async _createUserOperation (transaction: Deferrable<TransactionRequest>): Promise<UserOperation> {
    ...
    if (this._isPhantom) {
      ...
      // calldata for WalletFactory deploy method
      // we won't be needing this._deploymentTransaction
      ...
      const ownerAddress = await this.signer.getAddress()
      const initCallData = new WalletFactory(this.provider!).getDeployTransactionCallData(ownerAddress, this.entryPoint.address, fallbackHandlerAddress, 0)
